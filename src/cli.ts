...
program
  ...
  .option(
    '--output <file>',
    'Output file path (defaults to reports/{owner}-{repo}-{date}.yaml)'
  )
  .action(async (options) => {
    ...
    try {
      ...
      if (!token) {
        console.warn(
          '⚠️  No GitHub token provided. API rate limits will be very restrictive.'
        );
        console.warn(
      ...
      console.log(
        `📋 Validating repository ${options.owner}/${options.repo}...`
      );
      ...
      if (!isValidRepo) {
        throw new Error(
          `Repository ${options.owner}/${options.repo} not found or not accessible`
        );
      }
      ...
      // Write to file
      const outputPath =
        options.output || getDefaultOutputPath(options.owner, options.repo);
      ...
      console.log('\n📊 Summary:');
      console.log(
        `   • Total PRs analyzed: ${report.analysis.totalPullRequests}`
      );
      console.log(
        `   • Average lines changed: ${report.summary.averageLinesChanged}`
      );
      console.log(
        `   • Average files changed: ${report.summary.averageFilesChanged}`
      );
      console.log(
        `   • PRs with reviews: ${report.summary.pullRequestsWithReviews}`
      );
      console.log(
        `   • PRs with comments: ${report.summary.pullRequestsWithComments}`
      );
